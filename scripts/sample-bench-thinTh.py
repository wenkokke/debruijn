...
def sample_bench_thinTh(i: int, j: int, m: int, verbose: bool = False) -> Iterator[str]:
  m_pop = 2**m-1
  if verbose and m_pop < i:
    warn(f"sample_bench_thinTh: population {m_pop} is smaller than sample size {i}")
  for nm in sample(range(1,m_pop), k=min(m_pop,i)):
    nm_str = f"0b{{0:>0{m}b}}".format(nm)
    n = m - nm.bit_count()
    n_pop = 2**n-1
    if verbose and n_pop < j:
      warn(f"sample_bench_thinTh: population {n_pop} is smaller than sample size {j}")
...
if __name__ == "__main__":
  ...
  print("[" + ", ".join(sample_bench_thinTh(args.count1, args.count2, args.size, verbose=args.verbose)) + "]")
