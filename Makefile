...
.PHONY: bench
bench: \
  data/bench-space-fast.csv \
  data/bench-space-safe.csv \
  data/bench-time-fast.csv \
...
data/bench-time-fast-integer-thinTh-samples1-cyberglot.csv: $(SOURCE_FILES)
  time cabal run bench-time -- --flags=-safe --flags=-thinning-as-word 'Bench.Data.DeBruijn.Thinning/' --csv data/bench-time-fast-integer-thinTh-samples1-cyberglot.csv

data/bench-time-fast-word-thinTh-samples1-cyberglot.csv: $(SOURCE_FILES)
  time cabal run bench-time -- --flags=-safe --flags=+thinning-as-word 'Bench.Data.DeBruijn.Thinning/' --csv data/bench-time-fast-word-thinTh-samples1-cyberglot.csv

data/bench-time-safe-thinTh-samples1-cyberglot.csv: $(SOURCE_FILES)
  time cabal run bench-time -- --flags=+safe --flags=-thinning-as-word 'Bench.Data.DeBruijn.Thinning/' --csv data/bench-time-safe-thinTh-samples1-cyberglot.csv
